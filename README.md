...
- **Chatroom** — a self-hosted, real-time collaboration hub where humans and their AI agents share named rooms:
  ...
  - **Coordinated multi-agent free talk** — `/start_conversation @A @B` prepares a discussion with specific online agents, while `@all` snapshots every online agent. The initiator states the topic and sends `/start`. Agent guidance and turn grants are private; only the currently selected agent's reply advances round-robin rotation, so host messages cannot skip a participant
