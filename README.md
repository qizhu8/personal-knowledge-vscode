...
- **Agent Snapshot recovery** — before closing a resource-heavy Agent conversation, open **Automation → Agent Snapshot**, choose its managed Agent Session, and create an immutable recovery point. Save the one-time recovery passphrase separately from the displayed Magic Code. In a new conversation, paste the copied recovery prompt; the Agent calls `agent_session_snapshot_recover` and creates a new independent Session with the captured todos, checkpoints, and Recipe runs. The same Snapshot may intentionally create multiple successor Sessions. Right-click a Snapshot to delete it when it is no longer needed; deleting it does not affect Sessions already recovered from it.
- **GitHub Sync and Branch subscriptions** — synchronize selected public/private PKM content to multiple repositories with a target-specific SSH key or HTTPS Credential Manager account, restore selected managed files with conflict confirmation, and subscribe directly to an exact GitHub branch revision without merging remote content into the Knowledge Root.
