...
- **Top-level content privacy** — right-click a first-level Skill, Note, Paper, Prompt project, Package, Script, or Server group and choose **Set as Private**. A `🔒` prefix marks the folder, every descendant inherits the boundary, and private content is independently excluded from both the Subscription picker and Broker snapshot generation. Subfolders cannot override the top-level decision
