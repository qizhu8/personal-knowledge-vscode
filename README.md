...
- **Five focused workspaces** — use **Knowledge** for Skills, Notes, and Research; **Tools** for Prompts, Scripts, Packages, Environments, and Servers; **Automation** for Agent Sessions, Agent Snapshots, and Recipe Library; **Projects** for durable work and conversation threads; and **Settings** for MCP, Skill Router, and network sharing. Each workspace remembers its last route without changing existing content identities.
- **Recipe Library workbench** — design reusable Automation Recipes in a visual dependency graph or canonical JSON, configure typed boundaries and Script adapters, attach required Skills and Notes, validate immutable revisions, edit in a standalone Browser workspace, and follow durable branch, loop, subrecipe, background-command, or human-gate execution from Agent Sessions.
- **Agent Snapshot recovery** — before closing a resource-heavy Agent conversation, open **Automation → Agent Snapshot**, choose its managed Agent Session, and create an immutable recovery point. PKM automatically copies the one-time Recovery Prompt, encrypts the captured todos, checkpoints, and Recipe runs at rest with AES-256-GCM, and lets you rotate the recovery passphrase without changing the captured state. This built-in encryption prevents accidental plaintext disclosure but is not a substitute for a user-held encryption key against someone who has the PKM implementation. In a new conversation, paste the prompt; the Agent creates an independent successor Session. GitHub Sync can back up encrypted Snapshots only when explicitly selected for a Target (off by default); Broker publishing, subscriptions, and other Sync surfaces exclude them.
- **Automatic GitHub Sync and Branch subscriptions** — back up selected public/private PKM content when it changes, using each Target's interval as a minimum delay that coalesces rapid edits rather than a periodic forced sync. Background Git operations are serialized, unchanged content performs no Git work, schedule/error status remains visible, and durable per-account VS Code GitHub Authentication is stored in SecretStorage. Target-specific SSH keys and legacy HTTPS Credential Manager accounts remain supported. Restore selected managed files with conflict confirmation, or subscribe directly to an exact GitHub branch revision without merging remote content into the Knowledge Root.
