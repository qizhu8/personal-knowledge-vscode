...
assert.match(panel, /class="pkm-search-field server-search-field"/, "Server search uses the shared search field");
assert.match(css, /\.dash-hd \.server-search-field\{order:2;width:100%;min-width:0\}/, "Server search occupies its own row on narrow screens");
...
assert.match(panel, /function focusServerDashboard\(slug\)/);
assert.match(panel, /function prepareServerFocusGroups\(\)/);
assert.match(panel, /serverCache = servers \|\| \[\];\s*prepareServerFocusGroups\(\);/, "fresh Server data expands the focused Server's parent groups before rendering");
assert.match(panel, /copyPathMenu\(`pkm:\/\/servers\/\$\{encodeURIComponent\(slug\)\}`\)/, "Server cards copy a canonical slug locator");
assert.match(panel, /data-server-slug=/);
