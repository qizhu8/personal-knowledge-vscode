...
function renderMcpDashboard(data) {
  ...
  const processLabelKey = status.process.running ? status.process.pid ? 'config.runningPid' : 'config.running' : status.ready ? 'config.readyNotDetected' : 'config.setupRequired';
  const rows = [
  ...
  return `<section class="mcp-dashboard">
    ...
    <div class="mcp-runtime-note" ${status.process.detail === 'Generated server process detected.' ? 'data-i18n="config.processDetected"' : ''}>${esc(status.process.detail || '')}${!status.process.running && status.ready ? ' Stdio MCP servers start on demand; use MCP: List Servers to start pkm.' : ''}</div>
